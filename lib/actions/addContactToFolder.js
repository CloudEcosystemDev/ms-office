...
async function processAction(msg, cfg) {
  const self = this;
  self.logger.info('Action addContactToFolder started');
  const instance = new ApiClient(cfg, self);
  async function addContactToFolder(postRequestBody, folderId, meta) {
    self.logger.info('Going to add contact to folder %s', folderId);
    const actionUrl = `/me/contactFolders/${folderId}/contacts`;
