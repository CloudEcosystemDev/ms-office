const ApiClient = require("../apiClient");
const { getMetadata } = require("../utils/helpers");

/**
 * This method will be called from OIH platform providing following data
 *
 * @param msg - incoming message object that contains ``body`` with payload
 * @param cfg - configuration that is account information and configuration field values
 * @param snapshot - saves the current state of integration step for the future reference
 */
async function processTrigger(msg, cfg, snapshot = {}) {
  const self = this;
  self.logger.info('Trigger getContactsInSharedMailbox started');
  self.logger.info('Incoming msg: %j', msg);
  self.logger.info('Incoming cfg: %j', cfg);
  self.logger.info('Incoming snapshot: %j', snapshot);
  const instance = new ApiClient(cfg, self);

  // Set the snapshot if it is not provided
  snapshot.lastUpdated = snapshot.lastUpdated || new Date(0).getTime();
  const { snapshotKey, arraySplittingKey, syncParam, skipSnapshot } =
    cfg.nodeSettings;

  async function getContacts(instance, userId) {
    let contacts = [];
    const actionUrl = `/users/${userId}/contacts`;
    self.logger.info('Going to fetch first 100 contacts from url: %s', actionUrl);
    let result = await instance.get(`${actionUrl}?$top=100&$skip=0`);
    if (result.error){
      self.logger.error('Execution failed with code: %s, message: %s', result.error.code, result.error.message);
      throw { name: result.error.code, message: result.error.message };
    }
    if ("value" in result) {
      self.logger.info('Found %s contacts', result.value.length);
      contacts = contacts.concat(result.value);
    } else {
      self.logger.info("Missing value key in response!");
      self.logger.info(result);
    }

    // Loop through all pages
    let c = 0;
    while ("@odata.nextLink" in result) {
      self.logger.info('Found next page of contacts, going to fetch next 100 contacts');
      c += 100;
      result = await instance.get(`${actionUrl}?$top=100&$skip=${c}`);
      if ("value" in result) {
        self.logger.info('Found %s contacts', result.value.length);
        contacts = contacts.concat(result.value);
      } else {
        self.logger.info("Missing value key in response!");
        self.logger.info(result);
      }
    }
    return contacts;
  }

  async function getAllContacts(userId, from, until) {
    let contacts = await getContacts(instance, userId);
    self.logger.info(`${contacts.length} results found`);
    const filteredContacts = [];
    const length = contacts.length;
    for (let i = 0; i < length; i += 1) {
      if ("lastModifiedDateTime" in contacts[i]) {
        const timestamp = Date.parse(contacts[i].lastModifiedDateTime);
        if (timestamp >= from && timestamp <= until) {
          filteredContacts.push(contacts[i]);
        }
      } else {
        return contacts;
      }
    }

    return filteredContacts;
  }
  const mailbox = cfg.mailboxId || null;
  if (!mailbox) {
    emitError(new Error("No Mailbox Provided"));
  }
  // Get all contacts starting from last snapshot or all contacts if no snapshot exists
  const contacts = await getAllContacts(
    mailbox,
    snapshot.lastUpdated,
    Date.now()
  );

  self.logger.info(`Found ${contacts.length} new records.`);
  if (skipSnapshot) {
    return contacts;
  }
  if (contacts.length > 0) {
    contacts.forEach((elem) => {
      const newElement = {};
      delete elem.id;

      newElement.metadata = getMetadata(msg.metadata);
      newElement.data = elem;
      self.emit("data", newElement);
    });

    // Save current time in snapshot
    snapshot.lastUpdated = Date.now();
    self.logger.info(`New snapshot: ${JSON.stringify(snapshot)}`);
    self.emit("snapshot", snapshot);
  } else {
    self.emit("snapshot", snapshot);
  }

  /**
   * This method will be called from OIH platform if an error occured
   *
   * @param e - object containg the error
   */
  function emitError(e) {
    self.logger.error(`ERROR: ${e}`);
    self.emit("error", e);
  }
}

module.exports = {
  process: processTrigger,
};
